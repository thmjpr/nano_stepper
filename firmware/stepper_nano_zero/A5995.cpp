...
void A5995::begin()
{
	...
	digitalWrite(PIN_A5995_ENABLE, LOW);
	pinMode(PIN_A5995_ENABLE, OUTPUT);

	digitalWrite(PIN_A5995_MODE, LOW);
	pinMode(PIN_A5995_MODE, OUTPUT);
	...
	//setup the PWM for current on the A5995, set for low current
...
void A5995::enable(bool enable)
{
	...
	if (enabled == false)
	{
		WARNING("A5995 disabled");
		setDAC(0, 0); //turn current off
		GPIO_LOW(PIN_A5995_ENABLE);
		GPIO_LOW(PIN_A5995_MODE);
...
int32_t A5995::move(int32_t stepAngle, uint32_t mA)
{
	...
	//if we are reverse swap the sign of one of the angles
